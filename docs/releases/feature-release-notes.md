...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-07-31 | macOS 截图超时 | 主线程的其它回调暂时卡住时，ScreenCaptureKit 截图不会在超时后又接受迟到结果，`get_app_state` 的五秒保护上限保持有效。 | 截图同步桥改用单调时钟，并在每次 main run loop pump 后先判定截止时间再接受完成信号；补确定性的迟到信号回归和放宽间隔的端到端超时测试。 |
...
| 2026-07-22 | macOS Dev helper 生命周期 | 普通使用和仓库验证不会再意外弹出 Dev helper 的 Accessibility / Screen Recording 引导，也不会把验证启动的隐藏 Dev agent 留在后台。 | Dev onboarding 改为显式环境变量 opt-in；验证进程用唯一 owner token 回收自己启动的 app-agent，并拒绝终止已存在的 agent。 |
